...
def tls(X, Y, Z2, nX, PROJ, Formule_IC_TLS):
    ...
    print("Debug - d_hat:", d_hat)
    print("Debug - delta_d_hat:", delta_d_hat)
    print("Debug - delta_d_hat[:-1]:", delta_d_hat[:-1])
    print("Debug - any(delta_d_hat[:-1] != 0):", any(delta_d_hat[:-1] != 0))
    ...
    if True not in (delta_d_hat[:-1] == 0):
        ...
        print("Debug - b_m1 shape:", b_m1.shape)
        print("Debug - b_m1 sample:", b_m1[:5])  # 打印前5个值作为样本
        b_m2 = np.matrix(np.sqrt(1 - np.sum(b_m1**2, axis=1))).T
        print("Debug - b_m2 shape:", b_m2.shape)
        print("Debug - b_m2 sample:", b_m2[:5])  # 打印前5个值
        print("Debug - b_m2 isreal:", np.isreal(b_m2).all())
        print("Debug - b_m2 has zero:", (b_m2 == 0).any())
        ...
        if (False in np.isreal(b_m2)) | (True in (b_m2 == 0)) | (True in np.isnan(b_m2)):
            ...
            for i in range(m):
                ...
                print("Debug - arg_min:", arg_min)
                print("Debug - arg_max:", arg_max)
                print("Debug - tan(arg_min):", np.tan(arg_min))
                print("Debug - tan(arg_max):", np.tan(arg_max))
                print("Debug - beta_hat_inf[{i}]:", beta_hat_inf[i])
                print("Debug - beta_hat_sup[{i}]:", beta_hat_sup[i])
        ...
        print("Debug - beta_hat_inf:", beta_hat_inf)
        print("Debug - beta_hat_sup:", beta_hat_sup)
    else:    
